-- Databricks SQL: 2026年7月の完了済み注文から顧客ごとの最終注文を返す。
-- 同日ならorder_idの大きい注文を採用。金額NULLは0。
-- 架空の注文データ。実テーブルは不要。
-- Converted from T-SQL to Databricks SQL.
WITH orders AS (
    ...
    WHERE status = 'completed'
      AND order_date >= CAST('2026-07-01' AS DATE) /* converted from CONVERT(DATE, ..., 23) */
      AND order_date <= LAST_DAY(CAST('2026-07-01' AS DATE)) /* EOMONTH → LAST_DAY; CONVERT → CAST */
)
SELECT customer_id, order_id, order_date, COALESCE(amount, 0) /* ISNULL(expr,default) → COALESCE: DBSQL ISNULL is a boolean predicate */ AS amount
FROM ranked_orders
WHERE rn = 1
