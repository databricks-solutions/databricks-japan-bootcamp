...
# MAGIC ## 実行環境
# MAGIC
# MAGIC **このノートブック上で実行します**。各セルの `%sql` がそのまま LLM 呼び出しを実行します。
# MAGIC
# MAGIC ノートブック右上の接続先が **「サーバーレス」** になっていることを確認してください。
# MAGIC
# MAGIC - ❌ `Serverless Starter Warehouse`（SQLウェアハウス）→ 一部のセルが動かない
# MAGIC - ✅ **サーバーレス**（ノートブック用コンピュート）→ 全セル実行可能
# MAGIC
# MAGIC > 💡 補足: SQL エディターでも同じクエリは動きますが、画面遷移が増えるのでノートブック上で完結する方をおすすめします。

# COMMAND ----------

# MAGIC %md
# MAGIC ## ⚠️ 事前準備: 2 つの CSV をテーブルとしてアップロード
# MAGIC
# MAGIC このハンズオンでは、ECサイト「Brick EC」のサンプルデータ（**`gold_reviews`** と **`support_inquiries`**）を使います。
# MAGIC DE ハンズオンと同じ要領で、配布CSV を **UI からテーブルとしてアップロード** してください。
# MAGIC
# MAGIC ### 配布 CSV の場所
# MAGIC
# MAGIC Step 3 でクローンした Git フォルダ内:
# MAGIC ```
# MAGIC databricks-japan-bootcamp/databricks-data-ai-bootcamp/20260427/data/
# MAGIC   ├── gold_reviews.csv          (1,000行) ← このハンズオンで使用
# MAGIC   └── support_inquiries.csv     (300行)   ← 発展問題で使用
# MAGIC ```
# MAGIC
# MAGIC ### 手順（各 CSV ごとに実施）
# MAGIC
# MAGIC #### 1) CSV をローカルにダウンロード
# MAGIC - ホーム画面上部の検索欄に「**Second half**」と入力 → Enter
# MAGIC - `Bootcamp_osaka_second half_data` フォルダをクリック
# MAGIC - `gold_reviews.csv` の右側「**︙**（3点メニュー）」→ **ダウンロード**
# MAGIC - 同様に `support_inquiries.csv` もダウンロード
# MAGIC
# MAGIC #### 2) CSV をテーブルとしてアップロード（UI）
# MAGIC - 左メニュー **「カタログ」** → `workspace` → `bootcamp_osaka` スキーマを開く
# MAGIC - 右上 **「作成」→「テーブル」**
# MAGIC - `gold_reviews.csv` を **ドラッグ&ドロップ**
# MAGIC - カタログ: `workspace`、スキーマ: `bootcamp_osaka` を確認
# MAGIC - **「テーブルを作成」** をクリック → AI が提案するカラム説明があれば **承認**
# MAGIC - 同じ要領で `support_inquiries.csv` もアップロード
# MAGIC
# MAGIC 完了したらノートブック上部のタブでこの `01_ai_data_processing` に戻り、次のセルを実行して件数を確認します。

# COMMAND ----------

# テーブルが作成されているか確認（gold_reviews: 1000、support_inquiries: 300）
result = spark.sql("""
