...
# MAGIC - 生成先は自分の `workspace.de_workshop` 配下の Volume（Free Edition・単独ユーザー、権限付与は不要）
# MAGIC
# MAGIC > Databricks Free Edition 前提。カタログ `workspace` / スキーマ `de_workshop` に生成する。
...
shared_schema = "de_workshop"                               # 自分の作業スキーマ（固定）
...
# MAGIC 履歴データ(landing)とマスター(master)用の Volume を自分のスキーマに作成する。
...
print("スキーマ・Volume 作成 完了（workspace.de_workshop）")
