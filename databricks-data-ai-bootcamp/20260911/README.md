...
3. **`10_bronze_autoloader` を実行（1 回目）**: Auto Loader が `landing` の各ファクトを増分取り込みし、`bronze_t_*` を作成（件数をメモ）。
4. **増分を体験**: **`admin/01_generate_incremental_data` を実行**して 7 月分の増分を `landing/<table>/batch=20260707/` に生成 → **`10_bronze_autoloader` を再実行（2 回目）** し、追加分だけが取り込まれること（Exactly-once）を確認。
5. **`20_silver` → `30_gold`** を順に実行（マスター結合・クレンジング・集計）。
6. **可視化 / オーケストレーション**: AI/BI ダッシュボード（Genie Code）と Lakeflow Jobs は説明資料の手順に沿って実施（ダッシュボード更新タスクの SQL ウェアハウスは Free 既定の **Serverless Starter Warehouse** を使用）。
