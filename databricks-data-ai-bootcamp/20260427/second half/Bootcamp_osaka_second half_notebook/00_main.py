...
# MAGIC ## 1-1. 事前準備の確認
# MAGIC
# MAGIC このノートブックを開く前に、**スライドの手順** または **以下の説明** に従って次の3つを完了してください:
# MAGIC
# MAGIC | # | やること | スライド |
# MAGIC |---|---|---|
# MAGIC | 1 | `iot_data.csv` をワークスペースから自分のPCにダウンロード | #42-44 |
# MAGIC | 2 | カタログ `workspace` 配下に **スキーマ `bootcamp_osaka`** を UI で作成 | #45-48 |
# MAGIC | 3 | スキーマ画面の「作成 → テーブル」から `iot_data.csv` をアップロードして **テーブル `iot_data`** を作成 | #49-52 |
# MAGIC
# MAGIC ### スライドを使わずノートブックだけで進める場合の手順
# MAGIC
# MAGIC #### 1) CSV をダウンロード
# MAGIC - ホーム画面上部の検索欄に「**Second half**」と入力 → Enter
# MAGIC - `Bootcamp_osaka_second half_data` フォルダをクリック
# MAGIC - `iot_data.csv` の右側「**︙**（3点メニュー）」→ **ダウンロード**
# MAGIC
# MAGIC #### 2) スキーマを作成（UI）
# MAGIC - 左メニュー **「カタログ」** → `workspace` カタログを選択
# MAGIC - 右上 **「スキーマを作成」** をクリック
# MAGIC - スキーマ名: **`bootcamp_osaka`**（全員共通） → 作成
# MAGIC
# MAGIC #### 3) CSV をテーブルとしてアップロード（UI）
# MAGIC - 作成された `bootcamp_osaka` スキーマ画面で右上 **「作成 → テーブル」**
# MAGIC - 先ほどダウンロードした `iot_data.csv` を **ドラッグ&ドロップ**
# MAGIC - カタログ: `workspace`、スキーマ: `bootcamp_osaka` であることを確認
# MAGIC - **「テーブルを作成」** をクリック → AI が提案するカラム説明があれば **承認**
# MAGIC - テーブル `workspace.bootcamp_osaka.iot_data` が作成される
# MAGIC
# MAGIC 完了したら **ノートブック上部のタブ** でこの `00_main` に戻ってください。
# MAGIC 次のセルを実行して、テーブルが正しく作成されているか確認します。
...
# MAGIC | 種類 | 名前 | 定義 |
# MAGIC |---|---|---|
# MAGIC | dimension（切り口） | `device_type` / `location` | デバイス種別・地域 |
# MAGIC | measure（指標） | `total_readings` | 読み取り件数 |
# MAGIC | measure（指標） | `critical_count` | ステータスが critical の件数 |
# MAGIC | measure（指標） | `avg_temperature` | 平均気温 |
...
# MAGIC
# MAGIC さっきまで手作業でやった Bronze → Silver → Gold を、**3 タスク直列の Job** にまとめて自動化します。
# MAGIC タスクには **専用の ETL ノートブック**（`01_bronze.py` / `02_silver.py` / `03_gold.py`）を使います。
# MAGIC （`00_main.py` は学習用にいろいろ含んでいるので、Job では使いません）
# MAGIC
...
# MAGIC ## 2-2. ジョブ名を設定
# MAGIC
# MAGIC - ジョブ名: **`DE_Pipeline`**（全員同じでOK：Free Edition は環境が分離されています）
# MAGIC
# MAGIC ## 2-3. タスク1を追加：Bronze 取込
# MAGIC
# MAGIC | 項目 | 値 |
# MAGIC |---|---|
...
# MAGIC | タスクの種類 | ノートブック |
# MAGIC | ソース | Workspace |
# MAGIC | パス | **`01_bronze.py`** を選択（同じフォルダ内）|
# MAGIC | コンピュート | サーバーレス |
# MAGIC
# MAGIC ## 2-4. タスク2を追加：Silver 変換
# MAGIC
# MAGIC 「**+ タスクを追加**」をクリック
# MAGIC
# MAGIC | 項目 | 値 |
...
# MAGIC | タスクの種類 | ノートブック |
# MAGIC | ソース | Workspace |
# MAGIC | パス | **`02_silver.py`** を選択 |
# MAGIC | コンピュート | サーバーレス |
# MAGIC
# MAGIC ## 2-5. タスク3を追加：Gold 集計
# MAGIC
# MAGIC 「**+ タスクを追加**」をクリック
# MAGIC
# MAGIC | 項目 | 値 |
# MAGIC |---|---|
# MAGIC | タスク名 | `gold_aggregation` |
# MAGIC | 依存先 | **`silver_transform`** を選択 |
# MAGIC | タスクの種類 | ノートブック |
# MAGIC | ソース | Workspace |
# MAGIC | パス | **`03_gold.py`** を選択 |
# MAGIC | コンピュート | サーバーレス |
# MAGIC
# MAGIC ## 2-6. 手動実行してみる
# MAGIC
# MAGIC 右上の **「今すぐ実行」** をクリック。
# MAGIC 3 タスクすべてが緑のチェックマークになれば成功です。
# MAGIC
# MAGIC ```
# MAGIC bronze_ingestion ─→ silver_transform ─→ gold_aggregation
# MAGIC ```
# MAGIC
# MAGIC ## 2-7. （余裕があれば）スケジュールを追加
# MAGIC
...
# MAGIC %md
# MAGIC # Step 3: カタログ + Genie で結果を確認（5分）
# MAGIC
# MAGIC Job の実行が成功したら、**Job が作った Bronze / Silver / Gold テーブル** をカタログで確認し、
# MAGIC Gold テーブルに対して Genie で質問してみます。
# MAGIC
# MAGIC ## 3-1. カタログエクスプローラーで自分のテーブルを見る
# MAGIC
# MAGIC 1. 左メニュー **「カタログ」** をクリック
# MAGIC 2. `workspace` > `bootcamp_osaka` を展開
# MAGIC 3. 以下のテーブルが揃っているはずです：
# MAGIC    - `iot_data`（事前準備でアップロード）
# MAGIC    - `iot_bronze` / `iot_silver` / `iot_gold`（手作業 + Job で作成）
# MAGIC 4. `iot_silver` をクリックして **「履歴」タブ** を開くと、Time Travel で作成した複数バージョン + Job 実行による更新が見えます

# COMMAND ----------

# MAGIC %md
# MAGIC ## 3-2. Genie で Gold + メトリックビューに質問
# MAGIC
# MAGIC 前半で体験した Genie に、**自分で作った Gold テーブル + メトリックビュー** を使わせてみます。
# MAGIC
# MAGIC 1. 左メニューから **「Genie」** をクリック
# MAGIC 2. 右上 **「新規」** → Genieスペース作成
# MAGIC 3. カタログ: `workspace`、スキーマ: `bootcamp_osaka`
# MAGIC 4. 以下の **2 つ** を選択して作成:
# MAGIC    - **`iot_gold`** （テーブル）
# MAGIC    - **`iot_metrics`** （メトリックビュー） ← Step 1-6 で作成した指標定義
...
# MAGIC
# MAGIC > 「**critical_count** が一番多い `device_type` × `location` の組み合わせは？」
# MAGIC
# MAGIC > 「`device_type` ごとの `total_readings` を棒グラフで見せて」
# MAGIC
# MAGIC ### メトリックビューの効果
# MAGIC
# MAGIC `critical_count` や `total_readings` のような業務指標を質問に含めると、Genie は **メトリックビューで定義した計算式**をそのまま使って回答します。
# MAGIC これによって「人によって `critical_count` の数え方が違う」というバラつきが起きません。
# MAGIC
