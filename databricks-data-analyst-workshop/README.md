...
手順 7 で取り込んだ Git フォルダの `query/` 配下にある SQL クエリファイル（拡張子 `.dbquery.ipynb`）をワークスペース上で直接開き、テーブルが正しく作成されていることを確認します。

### 8-1. ローデータを確認

1. サイドバーの **ワークスペース** から `databricks-japan-bootcamp` > `databricks-data-analyst-workshop` > `query` を開く
2. `ローデータ確認.dbquery.ipynb` をクリックして開く
...
`query/` 配下の以下 2 本の SQL クエリファイルも、同様に開いて **すべて実行** をクリックします。

- `性別・商品カテゴリ別売上高・比率.dbquery.ipynb`  
  性別 × 商品カテゴリでの売上高と、性別ごとの売上比率（%）を算出
- `地域・商品カテゴリ別売上高・比率.dbquery.ipynb`  
...
└── query/
    ├── ローデータ確認.dbquery.ipynb
    ├── メトリクスビューのクエリ.dbquery.ipynb
    ├── 性別・商品カテゴリ別売上高・比率.dbquery.ipynb
    └── 地域・商品カテゴリ別売上高・比率.dbquery.ipynb
```
