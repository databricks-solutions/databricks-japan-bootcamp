...
### 8-2. 集計クエリ・メトリクスビューを実行

`query/` 配下の以下 3 本の SQL ファイルも、同様に開いて **すべて実行** をクリックします。
...
- `メトリクスビューのクエリ.sql`  
  `orders_metric_view` メトリクスビューに対して `MEASURE()` で集計値を取得

> **メトリクスビューのリネージを記録するために、`メトリクスビューのクエリ.sql` も忘れず実行しましょう。**  
> Unity Catalog のリネージはクエリを実行した時点で記録されます。`メトリクスビューのクエリ.sql` を実行することで、`orders_metric_view` と元テーブル（`transactions` / `users` / `products`）の間のリネージが Catalog Explorer 上に記録され、ワークショップ本編でリネージグラフを観察できる状態になります。

実行後、`orders_metric_view` のリネージグラフ画面を開き、下図のようにリネージが記録されていることを確認します。

**リネージグラフ画面へのアクセス方法:**

1. サイドバーの **カタログ** から **カタログエクスプローラ** を開く
2. `data_analyst_workshop` > `bricksmart` > **メトリクスビュー** > `orders_metric_view` を選択
3. 詳細画面の **リネージ** タブを開き、**リネージグラフを表示** をクリック

<img src="image/8-2-1.png" alt="orders_metric_view のリネージグラフ" width="720">

いずれもエラーなく結果が表示され、リネージグラフが想定どおりに表示されていれば、ワークショップで利用するデータが正しく揃っている状態です。
