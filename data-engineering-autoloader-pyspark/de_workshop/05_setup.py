...
# MAGIC - `master` Volume にアップロード済みのマスター Parquet を Bronze マスターテーブルとして取り込み
# MAGIC
# MAGIC > **事前に必要な手動アップロード**（初回セットアップ手順を参照）
# MAGIC > - マスター 3 ファイル → `master` Volume の各テーブル名フォルダ配下
# MAGIC > - ファクト 4 ファイル → `landing` Volume の各テーブル名フォルダ配下
# MAGIC > 先に Volume を作成する必要があるため、**このノートブックを一度実行して Volume を作成 → ファイルをアップロード → 再実行**、の順で進める。
...
landing         = f"/Volumes/{catalog}/{schema}/landing"       # ファクト Parquet を手動アップロード
master          = f"/Volumes/{catalog}/{schema}/master"        # マスター Parquet を手動アップロード
...
# MAGIC ファイルはこの 3 つのうち `landing` と `master` に手動アップロードする。
...
print("——— この後、以下のパスにサンプル Parquet を手動アップロードしてください ———")
print(f"  マスター: {master}/<m_customers | m_merchants | m_payment_methods>/")
print(f"  ファクト: {landing}/<t_payments | t_point_transactions | t_charges | t_login_events>/")

# COMMAND ----------

# MAGIC %md
# MAGIC ## マスターテーブルの取り込み
# MAGIC `master` Volume にアップロードしたマスター Parquet を読み、自分のスキーマに Bronze マスターテーブルとして保存する。
...
def load_master(name: str):
    ...
    if not _has_files(src):
        print(f"⚠ 未アップロード: {src} — マスター Parquet をアップロード後に再実行してください")
        return
...
def add_pk(table_name: str, pk_col: str):
    ...
    if not _table_exists(table_name):
        print(f"スキップ（テーブル未作成）: {full} — マスターをアップロード後に再実行してください")
        return
...
# MAGIC   ├─ Volume : landing      ← ファクト Parquet を手動アップロード（この後 Bronze で取り込む）
# MAGIC   ├─ Volume : master       ← マスター Parquet を手動アップロード（上で取り込み済み）
